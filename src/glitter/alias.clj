               ...
               (with-meta
                 {:glitter/context
                  {:alias ~alias
                   :data (first args#)}}))))
         {:glitter/alias ~alias})
      `(with-meta (fn ~attr-map ~@body) {:glitter/alias ~alias}))))

(defn register!
  "Register an alias. Associates the alias key `k` with the function `f`:

   ```clj
   (glitter.alias/register! :ui/a custom-link)
   ```"
  [k f]
...
(defmacro defalias
  ...
  The global registry is available through `glitter.alias/get-registered-aliases`."
  [alias & forms]
