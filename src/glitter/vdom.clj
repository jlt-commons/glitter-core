(ns ^:no-doc glitter.vdom
  (:require [glitter.hiccup-headers :as h])
  #?(:cljs (:require-macros [glitter.vdom])))

;; Ported from replicant.vdom (https://github.com/cjohansen/replicant),
...
(defmacro from-hiccup [headers attrs children children-ks n-children]
  (if (:ns &env)
    `(let [headers# ~headers]
       (js/Array. (h/tag-name headers#) (h/rkey headers#) (h/classes headers#) ~attrs ~children ~children-ks (boolean (:glitter/unmounting (h/attrs headers#))) (h/sexp headers#) nil nil ~n-children))
    `(let [headers# ~headers]
       [(h/tag-name headers#) (h/rkey headers#) (h/classes headers#) ~attrs ~children ~children-ks (boolean (:glitter/unmounting (h/attrs headers#))) (h/sexp headers#) nil nil ~n-children])))
